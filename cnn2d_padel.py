# -*- coding: utf-8 -*-
"""
Created on Fri Jul  1 23:00:47 2022
...
def load_dataset_group(directorio_dataset,normalize=True):
    ...
    trainX_ordenada = np.ones((trainX.shape[1],ventana,6,1))
...
def evaluate_model(trainX, trainy, testX, testy, test_deportistas, n_filters, epochs, batch_size):
    ...
    model = Sequential()
    model.add(Conv2D(filters=n_filters/2, kernel_size=(3,3), activation='relu', input_shape=(n_timesteps,n_features,1)))
    if(dos_conv):
...
def summarize_results(scores, filters, epochs, batch_size):
	...
	pyplot.title('Accuracy para Epoch=%d, Batch_size=%d, Filtros capa CNN1D=%d, Kernel=(3,3)' % (epochs, batch_size, filters[i]))
